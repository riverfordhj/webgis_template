...
export function AddJsonLayer(viewer, jsondata, isFlyTo) {
  ...
  dataPromise.then(function(dataSource) {
    ...
    if (isFlyTo) viewer.flyTo(dataSource);
  });
  return dataPromise;
}
...
function AddKmlLayer(viewer, isFlyTo) {
  ...
  var kmlPromise = Cesium.KmlDataSource.load(
    "//192.168.5.4/data/kml/yunnanshi.kml",
    kmlOptions
  );
...
function AddWMS(viewer) {
  var provider = new Cesium.WebMapServiceImageryProvider({
    url:
      "http://202.114.148.160:8000/arcgis/services/云南/MyMapService/MapServer/WMSServer?",
...
function getPosition(self) {
  ...
  var handler = new Cesium.ScreenSpaceEventHandler(
    self.cesiumObjs.viewer.scene.canvas
  );
  var ray;
  handler.setInputAction(function(event) {
    ray = self.cesiumObjs.viewer.scene.camera.getPickRay(event.endPosition);
    cartesian = self.cesiumObjs.viewer.scene.globe.pick(
      ray,
      self.cesiumObjs.viewer.scene
    );
    if (cartesian) {
      //将笛卡尔坐标转换为地理坐标
      ...
      //获取相机高度
      height = Math.ceil(
        self.cesiumObjs.viewer.camera.positionCartographic.height
      );
